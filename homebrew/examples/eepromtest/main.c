...
void main()
{
    ...
    if(maple_request_eeprom_read(eeprom) == 0)
    {
        ...
        video_draw_text(0, 80, rgb(255, 255, 255), "Writing new EEPROM...");
        ...
        {
            video_draw_text(0, 88, rgb(0, 255, 0), "Success!");

            // Finally, re-read to verify the update worked
            video_draw_text(0, 96, rgb(255, 255, 255), "Reading EEPROM again to verify...");

            // First, try to read, bail out of it fails.
            memset(eeprom, 0, 128);
            if(maple_request_eeprom_read(eeprom) == 0)
            {
                video_draw_text(0, 104, rgb(0, 255, 0), "Success!");
                ...
                video_draw_text(0, 112, rgb(255, 255, 64), eeprom_buf);
            }
            else
            {
                video_draw_text(0, 104, rgb(255, 0, 0), "Failed!");
            }
        }
        else
        {
            video_draw_text(0, 88, rgb(255, 0, 0), "Failed!");
        }
    }
    else
    {
